"""
tests/test_pymlkit.py — Full test suite for pymlkit.
...
- Public API (pymlkit.train, pymlkit.auto_train)
"""
...
class TestSchemaDetector:
    def test_detects_numeric(self, clf_df):
        from pymlkit.data.validator import SchemaDetector
...
class TestPreprocessor:
    def _run(self, df, target, algorithm="random_forest"):
        from pymlkit.data.loader import SchemaDetector, DataValidator, DataProfiler
...
class TestModelRegistry:
    ...
    def test_custom_registration(self):
        from pymlkit.registry.model_registry import ModelRegistry, AlgorithmMeta
...
class TestTrainer:
    def test_classification(self, clf_df):
        from pymlkit.train.trainer import Trainer
...
class TestPublicAPI:
    ...
    def test_auto_train_function(self, clf_df):
        import pymlkit
        model = pymlkit.auto_train(clf_df, target="label", time_budget=20, verbose=0)
        assert model is not None
    ...
    def test_load_model_function(self, clf_df, model_dir):
        import pymlkit
        model = pymlkit.train(clf_df, target="label", algorithm="random_forest",
